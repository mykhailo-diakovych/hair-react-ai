...
const getNewRotation = (
  startMousePos: DeltaPos,
  rotationCenter: DeltaPos,
  mouseDelta: DeltaPos,
  startStyles: StyleRot,
  snapAngle?: number
) => {
  const startVector = {
...
export const useRotate = (
  props: UseRotateProps
): [OnRotateMouseDown, boolean] => {
  const { styles } = props;
  ...
  const onMouseDown: OnRotateMouseDown = useCallback(
    (e) => {
      ...
      resizableCenter.current = topLeft2Center(
        clientRect.y,
        clientRect.x,
        clientRect.width,
        clientRect.height
      ...
      const onMouseMove = (e: MouseEvent) => {
        ...
        props.onRotate?.({
          style: { rotationDeg: newRotation.current },
          delta,
          totalDelta: totalDelta.current,
          nativeEvent: e
        });
      };
